...
int Map::LoadFile( char * filename )
{
	...
	for( int i = 0 ; i < no_of_buildings ; i++ )
	{
		...
		fscanf(f,"%f %f %f %f %d %d",&buildings[i].x1,&buildings[i].y1,&buildings[i].x2,&buildings[i].y2, &buildings[i].buildingType, &buildings[i].textureID );
	//	fprintf(stdout,"%f %f %f %f %d %d\n",buildings[i].x1,buildings[i].y1,buildings[i].x2,buildings[i].y2, buildings[i].buildingType, buildings[i].textureID );
		...
		buildings[i].y2 = tempy;
       	...
       	buildings[i].bx1 = (int)(map->length/2.0+map->buildings[i].x1)/map->blocksize+1;
	    buildings[i].bx2 = (int)(map->length/2.0+map->buildings[i].x2)/map->blocksize+1;
		buildings[i].by1 = (int)(map->breadth/2.0+map->buildings[i].y1)/map->blocksize+1;
		buildings[i].by2 = (int)(map->breadth/2.0+map->buildings[i].y2)/map->blocksize+1;
	...
	for( int i = 0 ; i < num_guards ; i++ )
	{
		...
		int texid;
		float botangle;
		fscanf( f , "%s %d %f %f %f %f %f %f" , buf , &texid , &x[0] , &x[1] , &x[2] , &x[3] , &x[4], &botangle );
		if( (guard[i] = new Guard( buf , texid , x[0] , x[1] , x[2] , x[3] , x[4], botangle, i )) == NULL )
		{
			printf("Out of RAM!\n");
			SDL_Quit();
			exit(-1);
...
//	glColor3f(.5,.5,.5);
	...
	if( display_lines )
	{
		...
		{
			glBegin( GL_LINES );
				glVertex3f(-length/2.0+i*blocksize,0.2,breadth/2);
				glVertex3f(-length/2.0+i*blocksize,0.2,-breadth/2);
			glEnd();
			glBegin( GL_LINES );
				glVertex3f(length/2.0,0.3,-breadth/2+i*blocksize);
				glVertex3f(-length/2.0,0.3,-breadth/2+i*blocksize);
			glEnd();
		}
	}
