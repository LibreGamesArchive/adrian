	    ...
	    if (!m_FilePointer)
		 {
		    ...
		    sprintf(strMessage, "Unable to find the file: %s!",
			    strFileName);
		printf("%s\n", strMessage);
		Quit(1);
		}
	    ...
	    if (m_Header.version != 8)
		 {
		    ...
		    sprintf(strMessage,
			    "Invalid file format (Version not 8): %s!",
			    strFileName);
		printf("%s\n", strMessage);
		Quit(1);
		}
...
void CLoadMD2::ReadMD2Data() 
{
	...
	m_pFrames = new tMd2Frame[m_Header.numFrames];
	    ...
	    fread(m_pTexCoords, sizeof(tMd2TexCoord), m_Header.numTexCoords,
		  m_FilePointer);
	    ...
	    fread(m_pTriangles, sizeof(tMd2Face), m_Header.numTriangles,
		  m_FilePointer);
	...
	for (int i = 0; i < m_Header.numFrames; i++)
		...
		m_pFrames[i].pVertices =
		    new tMd2Triangle[m_Header.numVertices];
		...
		for (int j = 0; j < m_Header.numVertices; j++)
			...
			pVertices[j].vertex[0] =
			    pFrame->aliasVertices[j].vertex[0] *
			    pFrame->scale[0] + pFrame->translate[0];
			pVertices[j].vertex[2] =
			    -1 * (pFrame->aliasVertices[j].vertex[1] *
				  pFrame->scale[1] + pFrame->translate[1]);
			pVertices[j].vertex[1] =
			    pFrame->aliasVertices[j].vertex[2] *
			    pFrame->scale[2] + pFrame->translate[2];
} } } 
		    ...
		    for (int j = 0; j < (int)strName.length(); j++)
			 {
			    ...
			    if (isdigit(strName[j])
				&& j >= (int)strName.length() - 2)
				 {
	...
	int j = 0, i = 0;
		...
		currentFrame.numOfFaces = m_Header.numTriangles;
		    ...
		    for (j = 0; j < currentFrame.numOfVerts; j++)
			 {
			currentFrame.pVerts[j].x =
			    m_pFrames[i].pVertices[j].vertex[0];
			currentFrame.pVerts[j].y =
			    m_pFrames[i].pVertices[j].vertex[1];
		...
		delete[] m_pFrames[i].pVertices;
		    ...
		    new CVector2[currentFrame.numTexVertex];
		    ...
		    if (strlen(g_3DModel.pMaterials[i].strFile) > 0)
			 {
			    ...
			    createTexture(g_Texture,
					  g_3DModel.pMaterials[i].strFile, i);
...
//                              loadTGA( g_3DModel.pMaterials[i].strFile, g_Texture[i] ); 
	    ...
	    g_3DModel.currentFrame =
	    g_3DModel.pAnimations[g_3DModel.currentAnim].startFrame;
	    ...
	    for (int i = 0; i < g_3DModel.numOfAnimations; i++)
		 {
		if ((strcmp(g_3DModel.pAnimations[i].strName, "stand") == 0)
		     || (strcmp(g_3DModel.pAnimations[i].strName, "stand0") ==
			 0))
			 {
			AnimNum_Stand = i;
			}
		
		else if (strcmp(g_3DModel.pAnimations[i].strName, "attack") ==
			 0)
			 {
			AnimNum_Attack = i;
			}
		
		else if (strcmp(g_3DModel.pAnimations[i].strName, "run") == 0
			 || strcmp(g_3DModel.pAnimations[i].strName,
				   "run0") == 0)
			 {
			AnimNum_Run = i;
			}
		
		else if ((strcmp(g_3DModel.pAnimations[i].strName, "death") ==
			  0)
			 || (strcmp(g_3DModel.pAnimations[i].strName, "deatha")
			     == 0)
			 || strcmp(g_3DModel.pAnimations[i].strName,
				   "death1") == 0
			 || strcmp(g_3DModel.pAnimations[i].strName,
...
} void Md2::AnimateMD2Model(t3DModel * pModel) 
{
	    ...
	    tAnimationInfo * pAnim =
	    &(pModel->pAnimations[pModel->currentAnim]);
	    ...
	    for (int j = 0; j < pFrame->numOfFaces; j++)
		 {
			...
			int vertIndex =
			    pFirstFrame->pFaces[j].vertIndex[whichVertex];
			
			    // Get the index for each texture coordinate for this face
			int texIndex =
			    pFirstFrame->pFaces[j].coordIndex[whichVertex];
			    ...
			    if (pFirstFrame->pTexVerts)
				 {
				    ...
				    glTexCoord2f(pFirstFrame->
						 pTexVerts[texIndex].x,
						 pFirstFrame->
						 pTexVerts[texIndex].y);
				}
...
void Md2::createTexture(unsigned int textureArray[], char *strFileName,
			  int textureID) 
{
...
int Md2::getNumFrames(Anim_Type t) 
{
	switch (t)
		 {
	case STAND:
		return g_3DModel.pAnimations[AnimNum_Stand].endFrame -
		    g_3DModel.pAnimations[AnimNum_Stand].startFrame;
	case ATTACK:
		return g_3DModel.pAnimations[AnimNum_Attack].endFrame -
		    g_3DModel.pAnimations[AnimNum_Attack].startFrame;
	case DEATH:
		return g_3DModel.pAnimations[AnimNum_Death].endFrame -
		    g_3DModel.pAnimations[AnimNum_Death].startFrame;
	case RUN:
		return g_3DModel.pAnimations[AnimNum_Run].endFrame -
		    g_3DModel.pAnimations[AnimNum_Run].startFrame;
		}
