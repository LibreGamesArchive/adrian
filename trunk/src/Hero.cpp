		...
		case HERO_RUNNING:
			{
				if( curx >= destx - 10 && curx <= destx + 10 )
				{
					if( cury <= desty + speed && cury >= desty - speed )
					{
						...
						else
						{
							destx = destx1;
							desty = desty1;
							Compute(destx,desty,destx,desty);
						}
					}
				}
		        ...
		        newbx = (int)(map->length/2.0+newx)/map->blocksize+1;
		        newby = (int)(map->breadth/2.0+newy)/map->blocksize+1;

				bool collision = false;
        		for( int i = 0 ; i < map->no_of_buildings ; i++ )
				{
		            if(
		 	           (((newbx >= map->buildings[i].bx1)&&(newbx <= map->buildings[i].bx2))&&
			           ((newby == map->buildings[i].by1)||(newby == map->buildings[i].by2)))||
						(((newby >= map->buildings[i].by1)&&(newby <= map->buildings[i].by2))&&
						((newbx == map->buildings[i].bx1)||(newbx == map->buildings[i].bx2)))
					)
					{
...
{
	if(((x < map->length/2.0)&&(x > -map->length/2.0))&&((y < map->breadth/2.0)&&(y > -map->breadth/2.0)))
			return false;
...
int Hero::Compute( float &dx , float &dy , float perfectx , float perfecty )
{
	...
	{
		int corner_no = 0, building_no = 0, buildingcorner_no = 0, count = 0;
    	...
    	for( int i = 0 ; i < map->no_of_buildings ; i++ )
		{
			...
			one = ((tempy1 - cury - slope * tempx1 + slope * curx) >= 0 );
			two = ((tempy1 - cury - slope * tempx2 + slope * curx) >= 0 );
			three = ((tempy2 - cury - slope * tempx2 + slope * curx) >= 0 );
			four = ((tempy2 - cury - slope * tempx1 + slope * curx) >= 0 );
			...
			if( temp == 1 )
				...
				corner_no = (1-a) * 1 + (1 - b) * 2 + (1 - c) * 3 + (1 - d) * 4;
			else
				corner_no = 0;
			...
			{
				case 1:
					check1 = (tempy1 - cury - mslope * tempx1 + mslope * curx >= 0 );
					check2 = (tempy1 - perfecty - mslope * tempx1 + mslope * perfectx >= 0 );
					break;
				case 2:
					check1 = (tempy1 - cury - mslope * tempx2 + mslope * curx >= 0 );
					check2 = (tempy1 - perfecty - mslope * tempx2 + mslope * perfectx >= 0 );
					break;
				case 3:
					check1 = (tempy2 - cury - mslope * tempx2 + mslope * curx >= 0 );
					check2 = (tempy2 - perfecty - mslope * tempx2 + mslope * perfectx >= 0 );
					break;
				case 4:
					check1 = (tempy2 - cury - mslope * tempx1 + mslope * curx >= 0 );
					check2 = (tempy2 - perfecty - mslope * tempx1 + mslope * perfectx >= 0 );
					break;
				default:
					check1 = false;
					check2 = false;
		...
		if(count == 1)
		{
			...
			{
				case 1:
					dx = map->buildings[building_no].x1 - map->blocksize;
					dy = map->buildings[building_no].y1 - map->blocksize;
					break;
				case 2:
					dx = map->buildings[building_no].x2 + map->blocksize;
					dy = map->buildings[building_no].y1 - map->blocksize;
					break;
				case 3:
					dx = map->buildings[building_no].x2 + map->blocksize;
					dy = map->buildings[building_no].y2 + map->blocksize;
					break;
				case 4:
					dx = map->buildings[building_no].x1 - map->blocksize;
					dy = map->buildings[building_no].y2 + map->blocksize;
					break;
				default:
					printf("ERROR ERROR IN INTERMEDIATE CALCULATION\n");
					destx1 = destx = dx;
					desty1 = desty = dy;
					break;
			}
