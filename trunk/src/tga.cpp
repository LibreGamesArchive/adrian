...
tImageTGA *LoadTGA(const char *filename)
{
	...
	if ((pFile = fopen(filename, "rb")) == NULL) {
		cout << "Could not open the tga file : " << filename << endl;
		return NULL;
	}
	...
	if (imageType != TGA_RLE) {
		...
		// Check if the image is a 16 bit image (RGB stored in 1 unsigned short)
			...
			for (int i = 0; i < width * height; i++) {
				...
				fread(&pixels, sizeof(unsigned short), 1,
				      pFile);
		...
		while (i < width * height) {
			...
			if (rleID < 128) {
				...
				while (rleID) {
					...
					fread(pColors, sizeof(byte) * channels,
					      1, pFile);

					// Store the current pixel in our image array
					pImageData->data[colorsRead + 0] =
					    pColors[2];
					pImageData->data[colorsRead + 1] =
					    pColors[1];
					pImageData->data[colorsRead + 2] =
					    pColors[0];

					// If we have a 4 channel 32-bit image, assign one more for the alpha
					if (bits == 32)
						pImageData->data[colorsRead +
								 3] =
						    pColors[3];
			...
			// Else, let's read in a string of the same character
				...
				fread(pColors, sizeof(byte) * channels, 1,
				      pFile);
