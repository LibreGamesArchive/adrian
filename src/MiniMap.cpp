...
int MiniMap::GenerateDisplayLists( void )
{
	...
	glBegin( GL_POLYGON );
		glTexCoord2f( 0 , 0 );
		glVertex2f( (map->length/2.0)/xconvfactor , (map->breadth/2.0)/yconvfactor );
		glTexCoord2f( 1 , 0 );
		glVertex2f( -(map->length/2.0)/xconvfactor , (map->breadth/2.0)/yconvfactor );
		glTexCoord2f( 1 , 1 );
		glVertex2f( -(map->length/2.0)/xconvfactor , -(map->breadth/2.0)/yconvfactor );
...
//		glBindTexture( GL_TEXTURE_2D , map->buildings[i].textureID );
		glBegin(GL_POLYGON);
//			glTexCoord2f( .25, 0);
			glVertex2f(map->buildings[i].x1/xconvfactor , -map->buildings[i].y1/yconvfactor);
//			glTexCoord2f( .25, .75 );
			glVertex2f(map->buildings[i].x1/xconvfactor , -map->buildings[i].y2/yconvfactor);
//			glTexCoord2f( .70, .75);
			glVertex2f(map->buildings[i].x2/xconvfactor , -map->buildings[i].y2/yconvfactor);
//			glTexCoord2f( .70, 0);
			glVertex2f(map->buildings[i].x2/xconvfactor , -map->buildings[i].y1/yconvfactor);
		glEnd();
	}
...
void MiniMap::Render( void )
{
	...
	{
		if( guard[i]->Alive )
			glVertex2f( guard[i]->curx/xconvfactor , -guard[i]->cury/yconvfactor );
	}
	glColor3f( 1 , 1 , 1 );
	...
	glTranslatef( camera->initx/xconvfactor , -( (camera->initz) )/yconvfactor , 0 );
	glRotatef( camera->angle*180.0/3.141 , 0 , 0 , 1 );
	float x,y;
